...
/**
 * Client IHM - Exercice 3 : Capture d'écran.
 */
public class ClientIHM extends JFrame {
    ...
    public ClientIHM() {
        super("Robi Control Center — Exercice 3"); // Titre mis à jour
    ...
    private void buildUI() {
        ...
        // On a retiré save et load
        commandBox  = new JComboBox<>(new String[]{"translate", "setColor", "setPosition", "sleep", "screenshot"});
        ...
        statusLabel = new JLabel("Connecté au serveur de capture");
    ...
    private void buildMenu() {
        ...
        // On ne garde que la capture d'écran pour l'Exo 3
    ...
    private void updateArgsPanel() {
        ...
        arg1.setVisible(true); arg2.setVisible(true);
        argsPanel.getComponent(0).setVisible(true); argsPanel.getComponent(2).setVisible(true);

        if (cmd.equals("screenshot")) {
            arg1.setVisible(false); arg2.setVisible(false);
    ...
    private void buildAndSend() {
        ...
        // Screenshot est toujours pour 'space'
        if (cmd.equals("screenshot")) rec = "space";
        ...
        new Thread(() -> {
            try (Socket socket = new Socket(HOST, PORT);
                ...
                SwingUtilities.invokeLater(() -> {
                    ...
                    if (response != null) {
                        // Detection spécifique Exo 3 : Capture d'écran Base64
                        if (!response.startsWith("[") && response.length() > 500) {
                            showScreenshot(response);
                        } else {
                            // Écho classique pour les mouvements
                            executeFromJson(response, env);
                        }
                    }
                });
            } catch (Exception ex) {
                SwingUtilities.invokeLater(() -> statusLabel.setText("Serveur déconnecté ❌"));
            }
        }).start();
    }
    ...
    private void showScreenshot(String base64) {
        try {
            ...
            if (img != null) {
                JOptionPane.showMessageDialog(this, new JLabel(new ImageIcon(img)), "Rendu du Serveur", JOptionPane.PLAIN_MESSAGE);
            }
        } catch (Exception e) {
    ...
    private void executeFromJson(String json, Environment env) {
        ...
        for (String ns : nodeStrings) {
            ...
            for (String p : parts) tokens.add(p.replace("\"", "").trim());
    ...
    private Environment buildEnv(GSpace space, GRect robi) {
        ...
        // Commande screenshot vide côté client (c'est le serveur qui gère)
    ...
    private void setupKeyboardShortcuts() {
        KeyboardFocusManager.getCurrentKeyboardFocusManager().addKeyEventDispatcher(e -> {
            if (e.getID() != KeyEvent.KEY_PRESSED || inputField.isFocusOwner()) return false;
            switch (e.getKeyCode()) {
    ...
    private SNode buildSNode(List<String> tokens) {
        return new SNode() {
            ...
            public void setParent(SNode p) {} public void addChild(SNode c) {}
