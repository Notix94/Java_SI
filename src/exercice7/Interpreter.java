...
public class Interpreter {
	public Reference compute(Environment env, SNode next) {
		...
		if (op.equals("while")) {
		    ...
		    while (true) {
		        // réévaluer la condition à chaque tour
		        Reference cond = compute(env, next.get(1));
		        if (!cond.isTrue()) break;  // sortir si condition fausse

		        // exécuter toutes les instructions du corps de la boucle
		        for (int j = 2; j < next.size(); j++) {
		            result = compute(env, next.get(j));
		        }
		    }
		    return result;
		}
		...
		if (receiver == null)
			throw new Error("Objet inconnu : " + next.get(0).contents());
