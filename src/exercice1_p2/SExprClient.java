...
public class SExprClient {
	...
	private static final String SCRIPT = "(space setColor white) (robi setColor red) "
			+ "(robi translate 180 0) (space sleep 500) " + "(robi translate 0 80)  (space sleep 500) "
			+ "(robi translate -180 0)(space sleep 500) " + "(robi translate 0 -80)";
	...
	private static void executeTokens(List<String> tokens, Environment env) {
		...
		if (ref == null) {
			System.err.println("Inconnu : " + receiverName);
			return;
		}

		if (methodName.equals("sleep")) {
			try {
				Thread.sleep(Long.parseLong(tokens.get(2)));
		...
		if (cmd == null) {
			System.err.println("Commande inconnue : " + methodName);
			return;
		}
	...
	private static SNode buildSNode(List<String> tokens) {
		return new SNode() {
			...
			@Override
			public java.util.List<SNode> children() {
